...
var Piece = function (options = {}) {
  this.color = DEFAULTS.COLOR;
	this.grid = this.initializePieceGrid(this.color);
...
Piece.prototype.initializePieceGrid = function(color){
	// ultimately, this will only draw the grid tiles based on which piece it is
	var grid = new Array(3).fill(new Array(3));
	for (var i = 0; i < 3; i++) {
		for (var k = 0; k < 3; k++) {
			grid[i][k] = new Tile(true, color, false, false);
		}
	}
	return grid
};
