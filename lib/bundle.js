...
/***/ function(module, exports, __webpack_require__) {
	...
	Game.randomPiece = function(){
	  return Game.piecesArr[Math.floor(Math.random() * (Game.piecesArr.length-1))];
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	Board.prototype.clearRows = function (rows) {
	  ...
	  this.clearColumns(rows);
	  this.grid = Board.transposeBoard(this.grid);
	};
	
	Board.prototype.clearColumns = function (cols) {
	  cols.forEach(function(clearCol){
	    for (var i = 0; i < 12; i++) {
	      this.grid[clearCol][i].color = "none";
	      this.grid[clearCol][i].full = false;
	    }
	  }.bind(this));
	};
